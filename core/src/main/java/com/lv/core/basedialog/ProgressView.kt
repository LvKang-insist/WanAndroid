...
class ProgressView @JvmOverloads constructor(
    ...
    private fun setupBounds(layoutWidth: Int, layoutHeight: Int) {
        ...
        mCircleBounds =
            if (!mFillRadius) { // Width should equal to Height, find the min value to setup the circle
...
// do something, like changing the color
    ...
    private fun updateBarLength(deltaTimeInMilliSeconds: Long) {
        if (mPausedTimeWithoutGrowing >= PAUSE_GROWING_TIME) {
            ...
            if (mTimeStartGrowing > mBarSpinCycleTime) { // We completed a size change cycle
...
//----------------------------------
    ...
    init {
        ...
        animationValue = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
